...
- **Hosting:** PRODUCTION deploys to a **Hostinger VPS (CyberPanel / OpenLiteSpeed)** via rsync-over-SSH from
  ...
  served to the VPS origin via the edge reverse-proxy, mirroring Mosquito Watch; **the edge rule LANDED and the
  public URL is LIVE as of 2026-07-17** (verified: `pharmeasy.in/fever-watch/mumbai/` 200, `robots: index,follow`,
  canonical `https://pharmeasy.in/fever-watch/mumbai/` - so production is publicly reachable AND indexable; it was
  previously pending with another
...
- [x] Production hosting: **LIVE at `https://pharmeasy.in/fever-watch/` as of 2026-07-17.** CyberPanel / OpenLiteSpeed VPS deploy (2026-07-02, `deploy-cyberpanel.yml`, rsync-over-SSH, `SITE_ENV=production`; github.io demoted to staging; repo = `pharmeasyMarketing/fever-watch`). The **pharmeasy.in edge reverse-proxy route** to the VPS origin (owned by another team) has LANDED: verified 200 + `robots: index,follow` + correct prod canonical, so the site is publicly reachable and indexable. `FW_PROD_BASE_URL` confirmed = the public URL (canonicals resolve to `pharmeasy.in/fever-watch/...`). The `--delete` rsync has run green many times. **NOTE: production is `workflow_dispatch` ONLY** - it is NOT triggered by push; `daily.yml`'s `dispatch-production` job fires it once a day, so a code fix pushed to master reaches STAGING immediately but the VPS only at the next daily (or a manual `gh workflow run deploy-cyberpanel.yml`). STILL OPEN: HARDEN (pin `burnett01/rsync-deployments` to a SHA - it currently warns that v7 is deprecated, use v8 - and add known_hosts host-key pinning).
