(function () {
  ...
  // Disease-page constants - byte-identical to build_site.py DIS_MECH / DIS_ABOUT (and the mobile.js twins).
  var DIS_MECH = {
  ...
  function boot(j) {
    ...
    render(); buildTicker();
    if (!DIS) buildDock();  // disease pages hide all share surfaces (per product); hubs keep them
  }
  ...
  function spyScroll() {
    ...
    var ids = DIS ? ["s-week", "s-why", "s-weather", "s-do", "s-trend", "s-about", "s-other", "s-faq"]
      : ["s-week", "s-why", "s-weather", "s-do", "s-trend", "s-other", "s-faq"], cur = ids[0];
  ...
  function mountTrend(c) {
    if (window.FeverWatchTrend) window.FeverWatchTrend.mount(document.getElementById("s-trend"), c, DATA, { mode: "desktop", disease: DIS });
  }
  ...
  // The above-fold prefix (searchHeroDisease + shell + toc + s-week[diseaseCard] + s-why[diseaseBreakdown])
  // MUST stay byte-identical to build_site.py _disease_pre_d and the mobile.js twins. Edit ALL together.
  function renderDisease() {
    var c = cityObj(state.cityId), disease = diseaseObj(DIS), cell = cellFor(c.id, DIS);
    app.innerHTML = searchHeroDisease(c, disease) +
      '<div class="shell">' + diseaseToc(disease) +
        '<section id="s-week">' + diseaseCard(c, disease, cell) + '</section>' +
        '<section id="s-why">' + diseaseBreakdown(c, disease, cell) + '</section>' +
        '<div class="main">' + diseaseSwitcherSection(c) + diseaseWeatherSection(c, disease) + doSection(c) +
          '<section id="s-trend" class="fwtrend-host"></section>' + diseaseSeasonSection(c) + aboutSection(disease) +
          methodSection() + diseaseOtherSection(c) + faqSection() + readsSection() + '</div></div>';
    wireLeaderboard();
    mountTrend(c);
    fillMethodExamples();
    if (state.methodOpen && window.FeverWatchMethod) window.FeverWatchMethod.wire(document.getElementById("methbody"));
  ...
  function searchHeroDisease(c, disease) {
    return '<section class="srch"><div class="srchin">' +
      '<h1>' + esc(disease.label) + ' risk in ' + esc(c.name) + ' today, in <em>one score</em>.</h1>' +
      '<p class="subtitle">A daily ' + esc(disease.label) + ' risk score for ' + esc(c.name) + ', blended from weather conditions, Google Search interest and PharmEasy lab signals.</p>' +
      '<div class="locwrap"><button class="loccard" data-act="combo">' + LOC_PIN + '<span class="locname">' + esc(c.name) + '</span>' +
        '<span class="locchange">Change <span class="loccaret" aria-hidden="true">▾</span></span></button>' +
        '<div class="combopanel' + (state.comboOpen ? ' open' : '') + '"><input id="cityinput" placeholder="Where are you from? Type a city" autocomplete="off"><div class="comboloc" data-act="useLoc">◎ Use my location</div><div class="combolist" id="combolist"></div></div>' +
      '</div>' +
  ...
  // Single-arc disease dial - byte-identical to build_site.py _disease_ring + the mobile.js twin.
  ...
  function diseaseWeatherSection(c, disease) { return '<section id="s-weather">' + diseaseWeatherCard(c, disease.family) + '</section>'; }
  function diseaseBreakdown(c, disease, cell) {
    ...
    return '<div class="card whycard discard-why"><h2 class="sechead">Why this ' + esc(disease.label) + ' score?</h2>' +
      '<p class="secsub">How each signal builds ' + esc(c.name) + '\'s ' + esc(disease.label) + ' score. <button class="knowmore" data-act="openMethod">Know more</button></p>' +
  ...
  function diseaseSeasonSection(c) {
    ...
    return '<section class="fwseason-sec"><div class="card fwseason"><h2 class="sechead">How this monsoon compares</h2>' +
      '<p class="seasonp">As of ' + fmtDate(DATA.generated_at) + ', ' + esc(c.name) + "'s " + esc(dl) + ' signal is <b>' + s.ty + '/100</b> - ' + s.phrase + ' the same week last monsoon (' + s.ly + "/100). Last season's high point came in the week of " + s.peak + '. Rain drives this number, so the picture shifts as the season moves - we refresh it daily.</p></div></section>';
  }
  function aboutSection(disease) {
    ...
    return '<section id="s-about"><div class="card"><h2 class="sechead">About ' + esc(disease.label) + '</h2><p class="aboutp">' + esc(a[0]) + '</p>' + link + '</div></section>';
  }
  ...
  function diseaseSwitcherSection(c) {
    ...
    return '<section><div class="card"><h2 class="sechead">Explore other fevers in ' + esc(c.name) + '</h2>' +
      '<p class="secsub">Each fever gets its own daily score. Pick one to see it.</p>' +
      '<div class="disswitch-row">' + chips + hub + '</div></div></section>';
  }
  function diseaseOtherSection(c) {
    var dl = diseaseObj(DIS).label;
    return '<section id="s-other"><div class="card"><h2 class="sechead">' + esc(dl) + ' risk in other cities</h2><p class="secsub">Top cities for ' + esc(dl) + ' today.</p>' +
      ...
      '<p class="lbnote" style="margin-top:8px"><a href="' + hubHref(c.id) + '">See all fevers in ' + esc(c.name) + '</a></p></div></section>';
  }
