...
def main() -> int:
    ...
    for d in diseases:
        ...
        for state, geo in states_with_geo:
            try:
                ...
                # _get() exhausted its rotation. Disambiguate via live quota: a tiny
                # state with no data raises the same RuntimeError as a real wipeout.
    ...
    # Carry-forward merge over an existing file: keep prior per-state series for any
    # disease/state we did not (re)fetch this run, overlay this run's results. Makes
    # a partial / quota-stopped run resumable on re-run.
