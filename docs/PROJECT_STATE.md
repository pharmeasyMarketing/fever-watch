...
>
> **2026-06-12 (SHARE-CARD PRODUCTION PORT - the #5 redesign SHIPPED into the pipeline):** the dual-QA'd,
...
>   untouched) AND `assets/img/share/{id}.jpg` (1080x1440 portrait, the WhatsApp share image; new gitignored
...
>   aa-matra, so it is never used) -> 2x -> LANCZOS -> opaque JPEG q85. Fail-loud: collects per-city errors and
>   exits nonzero (never publishes a partial set). The "Up from N last week" pill self-activates once
>   `data/history.json` has a day 4-10 days back (1 day old today -> pill omitted, layout re-pads).
...
src/
  ...
  build_share_cards.py  per-city share cards -> assets/img/og/{city}.jpg (1200x630) + assets/img/share/{city}.jpg
                    (1080x1440); SVG + HarfBuzz pre-shaping (textshape.py) + vendored resvg; JPEG q85
